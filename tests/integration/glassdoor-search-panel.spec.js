...
test.describe('Glassdoor Search Panel - DOM Selection', () => {
  let page;

  test.beforeEach(async ({ browser }) => {
    page = await browser.newPage();

    const htmlPath = path.join(__dirname, '../fixtures/glassdoor-job-search.html');
    const html = fs.readFileSync(htmlPath, 'utf-8');

    await page.route('https://www.glassdoor.com/**', route => {
    ...
    await page.goto('https://www.glassdoor.com/Job/detroit-director-software-engineering-jobs-SRCH_IL.0,7_IC1134644_KO8,37.htm');
  ...
  test('selects job description on keyboard shortcut', async () => {
    ...
    expect(selectedText).toContain('Why AAA Life');
  ...
  test('does not select content outside the description', async () => {
    ...
    expect(selectedText).not.toContain('Other content that should NOT be selected');
  });
